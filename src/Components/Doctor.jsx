...
const Doctor = ({doctor}) => {
    const {id, name, education, experience, registration, image, speciality, availability} = doctor;
    ...
    return (
       <div className="doctor-card p-8 rounded-2xl bg-white flex flex-col justify-between text-center md:text-start">
        ...
        <Link to={`doctor/${id}`} className='mt-5 block p-3 text-lg md:text-xl font-bold text-center transition ease-in-out duration-300 border bg-transparent text-blue-500 hover:text-white hover:bg-blue-500 w-full rounded-full'
        >View Details</Link>
